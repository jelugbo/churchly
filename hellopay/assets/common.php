...
function log_response($res,$donor,$parish,$method,$failed){
    $currency = ($method > 1 || $failed) ? $res->currency : $res->transactions[0]->amount->currency;
    $desc = ($method > 1 || $failed) ? $res->description : $res->transactions[0]->description;
    $card = ($method > 1 || $failed) ? $res->source->last4 : substr($res->payer->funding_instruments[0]->credit_card->number, -4);
    $amount = ($method > 1 || $failed) ? ($res->amount / 100) : $res->transactions[0]->amount->total;
    $ref = $res->id;
    $status = ($method > 1 || $failed) ? $res->status : $res->state;
    $give = new Give();
...
function create_fail_response($amount,$items,$card){
    $res = new stdClass();
    $res->currency='USD';
    $res->status='Failed';
    $res->description ='There was an exception processing this payment';
    $res->id =substr(str_shuffle(str_repeat($x='0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', ceil(10/strlen($x)) )),1,10);;
    $res->source->last4 = $card;
    $res->description ='There was an exception processing this payment for: '.get_description($items);
    $res->amount = $amount;
    return $res;
}
...
function notify_payer($id,$desc,$total,$state,$donor,$parish){
    ...
    $name = $user_db->getFname().' '.$user_db->getLname();
    $email = $user_db->getEmail();
    $status = (($state == "succeeded") || ($state == "approved")) ? "Successful" : "Failed";
...
function doPayPal($itemdata, $card,$total, $settings){
    ...
    error_log(print_r($res,true),3,"ex_paypal.txt");
    return $res;
//    try {
//        $payment->create($apiContext);
//    } catch (Exception $ex) {
