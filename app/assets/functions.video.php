...
function getVideoThumbnailByUrl($url, $format = 'small')
{
    ...
    elseif (false !== ($id = getYoutubeId($url))) {
        ...
        if ('medium' === $format) {
            ...
            //vid = "http://i.ytimg.com/vi/" + videoId + "/hqdefault.jpg";
        }
        return 'http://img.youtube.com/vi/' . $id . '/hqdefault.jpg';
    }
    return false;
...
/**
 * Returns Additional data for Media:
 *
 *      - youtube
 *      - daily motion
 *      - vimeo
 ...
 * This function can be used for creating video sitemaps.
 */
function getVideoApiData($temp){
    foreach($temp as $k => $v){
        if(is_array($v)){
            if($temp[$k]['Type'] == 3){
                $temp[$k]['MediaType'] = "Dailymotion";
                $temp[$k]['MediaId'] = getDailyMotionId($temp[$k]['Url']);
            }
            if($temp[$k]['Type'] == 4){
                $temp[$k]['MediaType'] = "Youtube";
                $temp[$k]['MediaId'] = getYoutubeId($temp[$k]['Url']);
            }
            if($temp[$k]['Type'] == 5){
                $temp[$k]['MediaType'] = "Vimeo";
                $temp[$k]['MediaId'] = getVimeoId($temp[$k]['Url']);
            }

            $temp[$k]['MediaThumb'] = getVideoThumbnailByUrl($temp[$k]['Url']);
            $temp[$k]['MediaLocation'] = getVideoLocation($temp[$k]['Url']);
        }
    }
    return $temp;
}
