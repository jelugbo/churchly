...
var JsDBSource = {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var filtered_data = $.grep(response, function (client) {
                    return (!filter.CurrencyId || client.CurrencyId === filter.CurrencyId)
                });
    ...
    deleteItem: function (item) {
        return $.ajax({
            ...
            data: {"Object":"give_parish_currency"}
        }).done(function(res) {
            $("#jsGrid").jsGrid("loadData"); // here we resolve promise with actual item
...
!function($) {
    ...
    GridApp.prototype.createGrid = function ($element, options) {
        ...
        GridApp.prototype.init = function () {
            ...
            var options = {
                fields: [
                    {name: "CurrencyId", type: "select", items: JsDBSource.currencies(), valueField: "Value", textField: "Code", title: "Currency", width: 50},
                    {type: "control"}
                ],
