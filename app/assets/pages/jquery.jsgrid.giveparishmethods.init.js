...
var JsDBSource = {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var filtered_data = $.grep(response, function (client) {
                    return (!filter.MethodId || client.MethodId === filter.MethodId)
                        && (!filter.Settings || client.Settings.indexOf(filter.Settings) > -1)
                        && (!filter.Enabled || client.Enabled.indexOf(filter.Enabled) > -1)
                });
    ...
    deleteItem: function (item) {
        return $.ajax({
            ...
            headers: {"authorization":"Bearer "+token},
            data: {"Object":"give_parish_methods"}
        }).done(function(res) {
            $("#jsGrid").jsGrid("loadData"); // here we resolve promise with actual item
...
!function($) {
    ...
    GridApp.prototype.createGrid = function ($element, options) {
        ...
        GridApp.prototype.init = function () {
            ...
            var options = {
                fields: [
                    {name: "MethodId", type: "select", items: JsDBSource.methods(), valueField: "Value", textField: "Code", title: "Payment Method", width: 50},
                    {name: "Settings", type: "textarea", width: 150},
                    {name: "Enabled", type: "checkbox", width: 30},
                    {type: "control"}
                ],
