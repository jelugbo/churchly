        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var filtered_data = $.grep(response, function (client) {
                    return (!filter.Enabled || client.Enabled === filter.Enabled)
                        && (!filter.UserId || client.UserId === filter.UserId)
                        && (!filter.GroupId || client.GroupId === filter.GroupId)
                });
...
!function($) {
    ...
    GridApp.prototype.createGrid = function ($element, options) {
        ...
        GridApp.prototype.init = function () {
            ...
            var options = {
                fields: [
                    {name: "UserId", type: "select", items: JsDBSource.users(), valueField: "Value", textField: "Fname", title: "First Name", editing: false},
                    {name: "UserId", type: "select", items: JsDBSource.users(), valueField: "Value", textField: "Lname", title: "Last Name", editing: false},
                    {name: "UserId", type: "select", items: JsDBSource.users(), valueField: "Value", textField: "Token", title: "Token", editing: false},
                    {name: "UserId", type: "select", items: JsDBSource.users(), valueField: "Value", textField: "Platform", title: "Platform", editing: false},
                    {name: "GroupId", type: "select", items: JsDBSource.groups(), valueField: "Value", textField: "Name", title: "Group", editing: false},
