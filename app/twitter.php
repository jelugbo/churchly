...
if (isset($_POST['post_data'])) {
	$db = new Twitter();
	if (!empty($_POST['TwitterId'])) $db = TwitterQuery::create()->findPk($_POST['TwitterId']);
	$db->setParishId($_SESSION["parish_id"]);
	$db->setHandleId($_POST['Handle']);
	$res = $db->save();
...
$obj_twitter = $obj_parish->getTwitters()->getFirst();
?>
				...
				<div class="container">
					...
					<div class="row">
						<div class="col-sm-12">
							...
							<h4 class="page-title">Twitter Setup</h4>
							<ol class="breadcrumb">
								<li><a href="index.php">Home</a></li>
								<li class="active">Twitter Setup</li>
							</ol>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="card-box">
								<h4 class="m-t-0 header-title">Twitter Setup</h4>
											...
											<form class="form-horizontal" action="#" data-parsley-validate novalidate method="post">
												...
												<div class="form-group">
													<label class="col-md-2 control-label">Twitter Handle</label>
													<div class="col-md-10">
                                                        <input type="text" name="Handle" required
                                                               value="<?php echo is_null($obj_twitter) ? ' ' : $obj_twitter->getHandleId() ?>"
                                                                  class="form-control">

												</div>
												</div>
												<?php if (is_null($obj_twitter)) { ?>
													<input type="text" name="new" class="hidden">
												<?php } ?>
												<input type="text"
												       value="<?php echo is_null($obj_twitter) ? ' ' : $obj_twitter->getValue() ?>"
												       name="TwitterId" class="hidden">
