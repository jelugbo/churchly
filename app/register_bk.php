	...
	if ($error_code == 1) {
		...
		if (!user_exist($_POST['Email'])) {
			...
			if (valid_username($_POST['Email'])) {
				// Create a unique salt for the user, Hash the salt and Generate the password hash
				$salt = generate_token(20);
				$salt_hash = '$2a$15$' . md5($salt);
				$hash = generateHash($salt_hash, $_POST['Password']);
				$hash2  = password_hash($_POST['Password']);
				// Create new user
				$data=array(
//					'Fname' => $_POST['Fname'],
//					'Lname' => $_POST['Lname'],
