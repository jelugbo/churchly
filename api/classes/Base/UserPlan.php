...
/**
 * Base class that represents a row from the 'user_plan' table.
 *
 * 
 *
 * @package    propel.generator..Base
 */
abstract class UserPlan implements ActiveRecordInterface 
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\UserPlanTableMap';
    ...
    /**
     * Initializes internal state of Base\UserPlan object.
     */
    ...
    /**
     * Compares this with another <code>UserPlan</code> instance.  If
     * <code>obj</code> is an instance of <code>UserPlan</code>, delegates to
     * <code>equals(UserPlan)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|UserPlan The current object, for fluid interface
     */
     ...
     * @return $this|\UserPlan The current object (for fluent API support)
     */
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[UserPlanTableMap::COL_VALUE] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[UserPlanTableMap::COL_NAME] = true;
        }
    ...
    public function setParams($v)
    {
        ...
        if ($this->params !== $v) {
            $this->params = $v;
            $this->modifiedColumns[UserPlanTableMap::COL_PARAMS] = true;
        }
    ...
    public function setCost($v)
    {
        ...
        if ($this->cost !== $v) {
            $this->cost = $v;
            $this->modifiedColumns[UserPlanTableMap::COL_COST] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : UserPlanTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            $this->value = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : UserPlanTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : UserPlanTableMap::translateFieldName('Params', TableMap::TYPE_PHPNAME, $indexType)];
            $this->params = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UserPlanTableMap::translateFieldName('Cost', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = UserPlanTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\UserPlan'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildUserPlanQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see UserPlan::setDeleted()
     * @see UserPlan::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildUserPlanQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                UserPlanTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[UserPlanTableMap::COL_VALUE] = true;
        if (null !== $this->value) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . UserPlanTableMap::COL_VALUE . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(UserPlanTableMap::COL_VALUE)) {
            $modifiedColumns[':p' . $index++]  = 'value';
        }
        if ($this->isColumnModified(UserPlanTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(UserPlanTableMap::COL_PARAMS)) {
            $modifiedColumns[':p' . $index++]  = 'params';
        }
        if ($this->isColumnModified(UserPlanTableMap::COL_COST)) {
            $modifiedColumns[':p' . $index++]  = 'cost';
        }

        $sql = sprintf(
            'INSERT INTO user_plan (%s) VALUES (%s)',
    ...
    {
        $pos = UserPlanTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['UserPlan'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['UserPlan'][$this->hashCode()] = true;
        $keys = UserPlanTableMap::getFieldNames($keyType);
        $result = array(
    ...
    {
        $criteria = new Criteria(UserPlanTableMap::DATABASE_NAME);

        if ($this->isColumnModified(UserPlanTableMap::COL_VALUE)) {
            $criteria->add(UserPlanTableMap::COL_VALUE, $this->value);
        }
        if ($this->isColumnModified(UserPlanTableMap::COL_NAME)) {
            $criteria->add(UserPlanTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(UserPlanTableMap::COL_PARAMS)) {
            $criteria->add(UserPlanTableMap::COL_PARAMS, $this->params);
        }
        if ($this->isColumnModified(UserPlanTableMap::COL_COST)) {
            $criteria->add(UserPlanTableMap::COL_COST, $this->cost);
        }
    ...
    {
        $criteria = ChildUserPlanQuery::create();
        $criteria->add(UserPlanTableMap::COL_VALUE, $this->value);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \UserPlan (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \UserPlan Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildUserPlan is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildUserPlan The current object (for fluent API support)
     */
    public function setUserSubscriptions(Collection $userSubscriptions, ConnectionInterface $con = null)
    {
        ...
        foreach ($userSubscriptionsToDelete as $userSubscriptionRemoved) {
            $userSubscriptionRemoved->setUserPlan(null);
        }
    ...
    protected function doAddUserSubscription(ChildUserSubscription $userSubscription)
    {
        ...
        $userSubscription->setUserPlan($this);
    }
    ...
    public function removeUserSubscription(ChildUserSubscription $userSubscription)
    {
        if ($this->getUserSubscriptions()->contains($userSubscription)) {
            ...
            $userSubscription->setUserPlan(null);
        }
     ...
     * Otherwise if this UserPlan is new, it will return
     * an empty collection; or if this UserPlan has previously
     * been saved, it will retrieve related UserSubscriptions from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in UserPlan.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
    ...
    {
        return (string) $this->exportTo(UserPlanTableMap::DEFAULT_STRING_FORMAT);
    }
