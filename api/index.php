...
$app->post('/verify', function (Request $request, Response $response, $args) {
    $req = $request->getParsedBody();
    $to = (isset($req['To'])) ? filter_var($req['To'], FILTER_SANITIZE_STRING) : ' ';
    $msg = filter_var($req['Msg'], FILTER_SANITIZE_STRING);
    $email = filter_var($req['Email'], FILTER_SANITIZE_STRING);
    $sent = (isset($req['To']) && isset($req['Msg'])) ? sendSMS($to,$msg) : false;
    $subject = "Churchlify - Verification Code";
    $sent_email = send_email($to,$email,$subject,$msg);
	$res  = ($sent || $sent_email->status) ?  $res = array("value"=>1):array("value"=>0);
    return $response->withJson($res,200);
});

$app->post('/notify', function (Request $request, Response $response, $args) {
    $req = $request->getParsedBody();
    $uid = filter_var($req['Uid'], FILTER_SANITIZE_STRING);
    $pid = filter_var($req['Pid'], FILTER_SANITIZE_STRING);
    $module = filter_var($req['Mod'], FILTER_SANITIZE_STRING);
    $subject = "Churchlify Mobile App - Notification from your member";
	$msg = get_notify_message($module, $uid);
	$emails = get_notify_emails($uid,$pid);
	$count = =
	foreach($emails as $to){
		$sent = send_email($to['name'],$to['email'],$subject,$msg);
		if ($sent->status) $count++;
	}
	$res  = ($count > 0) ?  $res = array("value"=>1):array("value"=>0);
    return $response->withJson($res,200);
});
...
$app->get('/parishes[/{church}]', function (Request $request, Response $response, $args) {
    ...
    $q = new ParishQuery();
    $data = $church_id ? $q->filterByEnabled(1)->findByChurchId($church_id) :  $q->filterByEnabled(1)->find();
    $res = empty($data->toArray()) ? array("value"=>0) : $data->toArray();
    return $response->withJson($res,200);
});
...
$app->post('/user/new', function (Request $request, Response $response, $args) {
    $req = $request->getParsedBody();
    $parish_id = filter_var($req['ParishId'], FILTER_SANITIZE_NUMBER_INT);
    $token = isset($req['PushId'])? filter_var($req['PushId'], FILTER_SANITIZE_STRING):'';
...
$app->put('/user/{id}', function (Request $request, Response $response, $args) {
    ...
    $req = $request->getParsedBody();
    $parish_id = filter_var($req['ParishId'], FILTER_SANITIZE_NUMBER_INT);
    $token = filter_var($req['PushId'], FILTER_SANITIZE_STRING);
